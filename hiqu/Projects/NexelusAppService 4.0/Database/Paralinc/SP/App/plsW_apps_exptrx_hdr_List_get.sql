...
if exists(select * from sys.procedures where name='plsW_apps_exptrx_hdr_List_get')
begin 
drop proc [plsW_apps_exptrx_hdr_List_get]
end
go

---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [plsW_apps_exptrx_hdr_List_get]    
  @company_code int                          
...
approved_by	,
finalise_flag	,
finalised_by, trx_type, reimbursment_flag, amount_home, payment_code	 

into #pld from pld_transactions where record_id in (select record_id from #transaction)    
...
--selerct * from #temp_pm_level2 where level2_key='MOBILE-APP-001'
...
--SELECT level2_key FROM #tamp_valid_level2 where level2_key='MOBILE-APP-001'
...
and resource_id in (SELECT resource_id FROM #temp_sup)                 
      -- SELECT resource_id FROM #temp_sup         
     --select 'Nonchargeable',* from #tamp_valid_level2 where level2_key='MOBILE-APP-001'         
insert into #tamp_valid_level2                 
(                 
level2_key,                 
level3_key,                 
...
INSERT #tmp_trx (                 
	...
	--AND ((h.approver_id is NULL) or (h.approver_id = ''))                 
	 ...
	 --select distinct * from #tamp_valid_level2 where level2_key='MOBILE-APP-001'
	INSERT #tmp_trx (                 
	resource_id,                 
	name_first,                 
	name_last,                 
...
INSERT #tmp_trx(                 
		...
		--AND d.trx_type BETWEEN @trx_type_form AND @trx_type_to                 
		--AND ISNULL(d.upload_flag,0) in (0, 9)                    
		AND h.record_id  in (select record_id from #transaction)
		AND d.transaction_id not in (select transaction_id from #tmp_trx)
...
select distinct record_id into #reocrds from #tmp_trx
 ...
 --select '000',* from #tmp_resources where record_id='m8d5d8b57c324ab0'
...
--FS20180609
--update t
--set t.approval_flag=1--CASE WHEN l2.trx_approval_required_flag =0 OR l3.trx_approval_flag = 0 THEN 1 ELSE t.approval_flag END
--from #tmp_resources tm inner join #pld t  ON tm.record_id=t.record_id
...
UPDATE #tmp_resources                 
  ...
  select @resource_id_loginid=ltrim(rtrim(loginid)) from pdd_resources where resource_id=@resource_id
    ...
    --select '1',* from #tmp_resources     where record_id='m8d5d8b57c324ab0'     
  update  #tmp_resources    
  set approved=0
...
if exists (select 1 FROM #tmp_resources where  IsNull(#tmp_resources.approval_flag,9) = 2 /*Rejected*/                 
	...
	BEGIN
		...
		SELECT * INTO #tmp FROM #pld pld WHERE record_id IN (select Distinct record_id FROM #recordID)
			--FS20180529
		...
		,r.resource_id manager,res.reports_to supervisor,tmp.finalise_flag finalise_flag,tmp.resource_id,tmp.level2_key 
		INTO #tmpL
			FROM #tmp tmp
	...
	--select * FROM #tmp_resources where record_id='m8d5c425aeecc784'
...
BEGIN
			...
			SELECT * INTO #tempR FROM #pld WHERE record_id IN (select Distinct record_id FROM #RecordIds)
			--FS20180529
		...
		WHERE record_id IN (select distinct record_id from #tempRe m  --partial approval is not required In override approval AS discussed with Mati 20180606
            ...
            and  exists(select record_id from 	#pld where record_id=tr.record_id and IsNull(approval_flag,9) IN (1,4) )/*CHANGE NABBASI08252018*/
...
--			SET  summary_flag = 9
--		WHERE record_id IN (select distinct record_id from #tempReports m
--								WHERE NOT EXISTS (select * from #tempReports 
--												WHERE ISNULL(apprv_id,'')=''  
--													  AND  (trx_approval_required_flag <> 0 
--													  OR  (trx_approval_required_flag <> 0 AND trx_approval_flag <> 0))
--													  AND record_id = m.record_id )) 
----------------------------------------------FS
...
WHERE record_id in (select distinct record_id from #transaction)     
...
AND #pld.record_id in (select distinct record_id from #plt_trx_hdr)        
------------------------------->>>------------------------------->>>        
  ...
  --and create_date > DATEADD(week,  -10, GETDATE())          
   ...
   update #data     
     ...
     set name_first =r.name_first
		...
		--FS20180609
	--UPDATE #pld  --need to auto approve transactions whose approval is not required, summary_flag=9 are those transactions
	--SET approval_flag = 1
	--WHERE record_id = (select record_id from #data where summary_flag=9) AND isnull(approval_flag,0) = 0
		--select * from #data where report_name='050762nexelus'
