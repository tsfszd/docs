/****** Object:  Trigger [pdd_level3_insert_update_delete]    Script Date: 07/14/2015 03:28:57 ******/
IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[pdd_level3_insert_update_delete]'))
DROP TRIGGER [dbo].[pdd_level3_insert_update_delete]
GO



/****** Object:  Trigger [dbo].[pdd_level3_insert_update_delete]    Script Date: 07/14/2015 03:29:12 ******/
SET ANSI_NULLS ON
GO
...
CREATE TRIGGER [dbo].[pdd_level3_insert_update_delete]               
   ON  [dbo].[pdd_level3]              
...
select @entity_id  =entity_type_id from  [pdd_entity_type] where entity_type_desc = 'Level3'    
 ...
 SET @action = 1; -- Set Action to Insert by default.    
  ...
  select top 0 d.company_code, d.level2_key,d.level3_key, d.org_unit,location_code,  resource_id, subscriber_id  into  #data from inserted d, pdd_apps_subscriber_info   
        
     if @action = 1     
     begin    
     insert into #data select d.company_code, d.level2_key,d.level3_key, d.org_unit,location_code,  resource_id, subscriber_id       from inserted d, pdd_apps_subscriber_info   
  end    
  else     
  begin    
  insert into #data select d.company_code, d.level2_key,d.level3_key, d.org_unit,location_code,  resource_id, subscriber_id       from deleted d, pdd_apps_subscriber_info       
  end    
 ...
 where not exists 
	 (
		...
		where primary_key = convert(varchar, d.company_code) + '~-~' + d.level2_key  + '~-~' + d.level3_key and 
