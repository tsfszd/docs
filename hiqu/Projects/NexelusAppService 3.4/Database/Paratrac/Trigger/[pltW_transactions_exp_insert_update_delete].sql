IF  EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[pltW_transactions_exp_insert_update_delete]'))
...
DECLARE @action int;  
  ...
  select  @sub_resource=resource_id from pld_transactions  where transaction_id=@transaction_id
    ...
    select top 0 i.transaction_id, i.company_code, resource_id ,i.[source] into  #data  from inserted i inner join pld_transactions p on p.transaction_id = i.transaction_id  
	 ...
	 if EXISTS(SELECT * FROM INSERTED where source like '%app%' ) 
		 begin 
			if dbo.fn_is_column_updated('pld_transactions_exp', 'source', COLUMNS_UPDATED())=1
			begin 
			insert into #data select i.transaction_id, i.company_code, s.resource_id ,i.[source] from inserted i inner join pld_transactions p on p.transaction_id = i.transaction_id ,#subscriber_IDS s              
			end
		 end
